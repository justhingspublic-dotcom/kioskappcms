...
(async () => {
  const browser = await puppeteer.launch({ executablePath: 'C:/Program Files/Google/Chrome/Application/chrome.exe', headless: 'new', defaultViewport: { width: 1512, height: 760 } });
  ...
  for (const [name, sel] of [['dz', '.danger-zone .page-help-btn'], ['sleep', '#settingsBody > .settings-card:nth-child(2) .page-help-btn'], ['idle', '#settingsBody > .settings-card:nth-child(4) .page-help-btn']]) {
    ...
    await page.screenshot({ path: path.join(__dirname, 'shots', 'settings-fit', 'help760-' + name + '.png') });
