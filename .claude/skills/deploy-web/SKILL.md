...
- **純顯示播放頁（2026-09-10）**：`/{site}/play/?device=機器名&key=金鑰`（機器名與金鑰帶過一次都記在瀏覽器 localStorage，之後裸網址 `/{site}/play/` 即可；`&fresh=1`＝被後台刪除後重新登錄；`&id=`可自訂機器編號，預設 `web-<機器名>`）。**sunrise 不設 PLAY_DEFAULT_DEVICE**（user 2026-09-10 16:15 指示不要預設機器名）：每台第一次開播放頁在螢幕上填名字；設了的話所有螢幕同一台機器、同一畫面。
